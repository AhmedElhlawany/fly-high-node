...
app.post("/api/forgot-password", async (req, res) => {
  ...
  try {
    data = readData();
    console.log('Read data:', data.users.map(u => u.email));
  } catch (err) {
...
app.post("/api/register", async (req, res) => {
  ...
  res.status(201).json({
    ...
    token,
    user: { id: newUser.id, email, name, phone, profilePhoto: newUser.profilePhoto },
  });
});
...
app.post("/api/login", async (req, res) => {
  ...
  res.json({
    ...
    token,
    user: { id: user.id, email, name: user.name, phone: user.phone, profilePhoto: user.profilePhoto },
  });
});
...
app.post("/api/users/:id/hotel-bookings", authenticateToken, (req, res) => {
  ...
  const { bookingId, hotelId, hotelName, city, rooms, totalCost, bookingDate, checkIn, checkOut, discountApplied } = req.body;

  if (!bookingId || !hotelId || !hotelName || !city || !rooms || !totalCost || !bookingDate || !checkIn || !checkOut) {
    return res.status(400).json({ error: "bookingId, hotelId, hotelName, city, rooms, totalCost, bookingDate, checkIn, and checkOut are required" });
  }
  ...
  user.bookedHotels.push({ bookingId, hotelId, hotelName, city, rooms, totalCost, fullName: user.name, phone: user.phone, bookingDate, checkIn, checkOut, discountApplied });
  writeData(data);
...
app.put("/api/users/:id/edit-hotel-booking", authenticateToken, (req, res) => {
  ...
  if (!bookingId || !updatedBooking) {
    return res.status(400).json({ error: "bookingId and updatedBooking data are required" });
  }
  ...
  const bookingIndex = user.bookedHotels.findIndex((b) => b.bookingId === bookingId);
  if (bookingIndex === -1) {
    return res.status(404).json({ error: "Booking not found" });
  }
  ...
  res.json({ message: "Booking updated successfully", updatedBooking: user.bookedHotels[bookingIndex] });
});
